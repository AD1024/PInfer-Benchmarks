...
event eReportLog: (epoch: tEpoch);
event eNotifyLog : (epoch: tEpoch, position: tPos, log: seq[tRid], sent: seq[tRid]);
...
machine Replicate {
    ...
    start state Boot {
        ...
        defer eReadRequest, eWriteRequest, eReportLog;
    }
    ...
    state Active {
        ...
        on eReportLog do (pld: (epoch: tEpoch)) {
            if (epoch <= pld.epoch) {
                announce eNotifyLog, (epoch = epoch, position = position, log = updates, sent = sent);
            }
        }
    ...
    state Recover {
        ...
        defer eWriteRequest, eReadRequest, eReportLog;
